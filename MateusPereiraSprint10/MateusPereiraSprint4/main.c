...
ISR(PCINT2_vect) // Interrupção 2 por mudança de pino
{
	...
	{
		marcha = 13; // Motor sentido anti-horário - R
	}
	else
	{
		marcha = 11; // Motor sentido horário - D
	}
...
ISR(ADC_vect) // Faz a Conversão ADC
{
	...
	if (!teste_porta (PINC, PC0)) // Pega a variação em PC0
	{
		...
		else
		{
			...
			{
				rotacao_motor_pwm = 102; //Regula a Rotação do motor para 10%, tem 1024 posições
			}
			...
			if(distancia_delta_sonar<300 && velocidade>20) //Alerta sonoro
			{
				PORTC |= 0b01000000;
			}
			else
...
ISR(USART_RX_vect)
{
	...
	itoa(aux_temp_eprom, &display_string, 10); //chamada da função para converter inteiros em string

	recebido = UDR0;
	
	if(recebido=='p') //Sirene Policial
	{
		PORTC ^= 0b00001000;
	}
	
	if(recebido=='l') //Limpa o valor da temperatura máxima e vai para o atual
	{
		//USART_Transmit(recebido);
		eeprom_write_byte(12,0);
	}
	if (recebido=='d') //Envia a máxima temperatura
	{
		...
		{
			//Precisa enviar uma posição por vez
			USART_Transmit(display_string[contador_imprime_serial]);
			//_delay_ms(1000);
		}
		
		USART_Transmit(' '); //Espaço para separar um valor e outro
	}
}
...
void USART_Init(unsigned int ubrr)
{
	...
	UCSR0B = (1<<RXCIE0)|(1<<RXEN0)|(1<<TXEN0); // Habilita a interrupção, receptor e transmissor
	UCSR0C = (1<<USBS0)|(3<<UCSZ00); // Ajusta o formato: 8 bits de dados e 2 de parada
}

void USART_Transmit(unsigned char data)
{
	while(!( UCSR0A & (1<<UDRE0))); // Espera limpar o registrador de transmissão
...
int main(void)
{
		...
		PORTD = 0b01001101;
		PORTC = 0b10110000; //Desabilita o pullup das Entradas, Alerta Sonoro e Sirene
